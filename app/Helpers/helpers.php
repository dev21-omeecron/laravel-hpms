...
class Helper
{
  public static function getMenuData()
  {
    ...
    $userRole = session('role'); // Get current user role

    // Filter menu items based on role
    if (isset($menuData->menu)) {
      $menuData->menu = self::filterMenuByRole($menuData->menu, $userRole);
    }

    return [$menuData];
  }

  private static function filterMenuByRole($menuItems, $userRole)
  {
    return array_filter($menuItems, function ($item) use ($userRole) {
      ...
      if (isset($item->submenu)) {
        $item->submenu = self::filterMenuByRole($item->submenu, $userRole);
        // If submenu becomes empty after filtering, hide parent item
